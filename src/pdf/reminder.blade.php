...
<body>
    ...
    <main>
        ...
        <p class="top">
            ...
            {{ $reminder->due_at->format('d.m.Y') }}<br>
            <span class="font-weight-bold">{{ __('interface.documents.reminder') }}:</span>
            {{ $reminder->invoice->type->dunnings->where('after', '<', $reminder->dunning->after)->count() + 1 }} /
            {{ $reminder->invoice->type->dunnings->count() }}<br>
            <span class="font-weight-bold">{{ __('interface.documents.invoice_number') }}:</span>
            {{ $reminder->invoice->number }}<br>
        ...
        <table id="dunnings" class="table mt-4 w-100">
            ...
            <tbody>
                @foreach ($reminder->invoice->positionLinks as $link)
                    <tr>
                        ...
                        <td style="width: 10%">
                            {{ $reminder->invoice->status !== 'refund' ? '' : '-' }}{{ number_format($link->position->amount, 2) }}
                            €</td>
                        <td style="width: 10%">{{ $link->position->quantity }}</td>
                        <td style="width: 10%">
                            {{ $reminder->invoice->status !== 'refund' ? '' : '-' }}{{ number_format($link->position->netSum, 2) }}
                            €</td>
                        <td style="width: 10%" class="bg-disabled">{{ $link->position->vat_percentage }} %</td>
                        <td style="width: 10%" class="bg-disabled">
                            {{ $reminder->invoice->status !== 'refund' ? '' : '-' }}{{ number_format($link->position->grossSum, 2) }}
                            €</td>
                    </tr>
                        ...
                        @switch ($discount->type)
                            @case ('percentage')
                                <tr>
                                    ...
                                    <td style="width: 10%">
                                        {{ $reminder->invoice->status == 'refund' ? '' : '-' }}{{ number_format($link->position->amount * ($discount->amount / 100), 2) }}
                                        €</td>
                                    <td style="width: 10%">{{ $link->position->quantity }}</td>
                                    <td style="width: 10%">
                                        {{ $reminder->invoice->status == 'refund' ? '' : '-' }}{{ number_format($link->position->netSum * ($discount->amount / 100), 2) }}
                                        €</td>
                                    <td style="width: 10%" class="bg-disabled">{{ $link->position->vat_percentage }} %</td>
                                    <td style="width: 10%" class="bg-disabled">
                                        {{ $reminder->invoice->status == 'refund' ? '' : '-' }}{{ number_format($link->position->grossSum * ($discount->amount / 100), 2) }}
                                        €</td>
                                </tr>
                            @break
                            ...
                            @case ('fixed')
                                ...
                                @default
                                    <tr>
                                        ...
                                        <td style="width: 10%">
                                            {{ $reminder->invoice->status == 'refund' ? '' : '-' }}{{ number_format($discount->amount, 2) }}
                                            €</td>
                                        <td style="width: 10%">{{ $link->position->quantity }}</td>
                                        <td style="width: 10%">
                                            {{ $reminder->invoice->status == 'refund' ? '' : '-' }}{{ number_format($discount->amount * $link->position->quantity, 2) }}
                                            €</td>
                                        <td style="width: 10%" class="bg-disabled">{{ $link->position->vat_percentage }} %</td>
                                        <td style="width: 10%" class="bg-disabled">
                                            {{ $reminder->invoice->status == 'refund' ? '' : '-' }}{{ number_format($discount->amount * $link->position->quantity * (1 + $link->position->vat_percentage / 100), 2) }}
                                            €</td>
                                    </tr>
                                @break
                            @endswitch
                    ...
                    @if ($reminder->dunning->fixed_amount)
                        <tr>
                            ...
                            <td style="width: 10%" class="bg-disabled">
                                {{ number_format($reminder->dunning->fixed_amount, 2) }} €</td>
                        </tr>
                    @endif
                    @if ($reminder->dunning->percentage_amount)
                        <tr>
                            <td>
                                ...
                                {{ __('interface.documents.late_payment_interest') }}:
                                {{ number_format($reminder->dunning->percentage_amount, 2) }} %
                            </td>
                            <td style="width: 10%">
                                {{ number_format($reminder->invoice->netSum * ($reminder->dunning->percentage_amount / 100), 2) }}
                                €</td>
                            <td style="width: 10%">-</td>
                            <td style="width: 10%">
                                {{ number_format($reminder->invoice->netSum * ($reminder->dunning->percentage_amount / 100), 2) }}
                ...
                <tfoot>
                    <tr>
                        ...
                        <td class="bg-primary text-white">
                            {{ $invoice->status !== 'refund' ? '' : '-' }}{{ number_format($reminder->invoice->netSum + ($reminder->dunning->fixed_amount ?? 0) + (!empty($reminder->dunning->percentage_amount) ? $reminder->invoice->netSum * ($reminder->dunning->percentage_amount / 100) : 0), 2) }}
                            €</td>
                        <td style="width: 10%"></td>
                        <td style="width: 10%"></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td class="bg-primary text-white border-0">
                            {{ $invoice->status !== 'refund' ? '' : '-' }}{{ number_format($reminder->invoice->grossSum + ($reminder->dunning->fixed_amount ?? 0) + (!empty($reminder->dunning->percentage_amount) ? $reminder->invoice->netSum * ($reminder->dunning->percentage_amount / 100) : 0), 2) }}
                            €</td>
                    </tr>
                </tfoot>
            </table>
            <p class="bottom">
                ...
                {{ $reminder->dunning->period }} {{ __('interface.units.days') }}<br>
                <span class="font-weight-bold">{{ __('interface.documents.payable_until') }}:</span>
                {{ $reminder->due_at->addDays($reminder->dunning->period)->format('d.m.Y') }}
